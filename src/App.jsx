...
import {
  openWeatherApiUrl_current,
  openWeatherApiUrl_forecast,
  openWetaherAppid,
} from "./api";
import "./App.css";
  ...
  const handleOnSearchChange = async (searchData) => {
    try {
      const currentweatherData = await fetch(
        `${openWeatherApiUrl_current}${searchData.value}&appid=${openWetaherAppid}&units=metric`
      );
      const forecastweatherData = await fetch(
        `${openWeatherApiUrl_forecast}${searchData.value}&appid=${openWetaherAppid}&units=metric`
      );
